...
class S3Service {
	...
	constructor(bucketName: string = config.S3.bucketName) {
	...
	async uploadFile(
		...
		try {
			const command = new PutObjectCommand({
				...
				ContentLength: typeof body === 'string' ? Buffer.byteLength(body) : undefined,
			});
	...
	async listFolders(prefix: string) {
		try {
			...
			const response = await s3Client.send(command);
			return (response.CommonPrefixes || []).map((p) => p.Prefix).filter((p): p is string => !!p);
		} catch (error) {
