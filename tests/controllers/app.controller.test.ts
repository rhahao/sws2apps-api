...
describe('getFeatureFlags', () => {
  ...
  it('should register a new installation if it does not exist', async () => {
    ...
    vi.spyOn(Service.API, 'installations', 'get').mockReturnValue([]);

    const mockFlags = { FLAG_NAME: true };

    (Service.API.evaluateFeatureFlags as Mock).mockResolvedValue(mockFlags);

    await Controller.App.getFeatureFlags(req as Request, res as Response);

    const { calls } = (Service.API.saveInstallations as Mock).mock;

    const installations = calls[0][0] as API.Installation[];
  ...
  it('should return feature flags but NOT save the installation if it already exists', async () => {
    ...
    const existingInstallations: API.Installation[] = [
      {
        id: installationId,
        user: userId,
    ...
    expect(Service.API.saveInstallations).not.toHaveBeenCalled();
